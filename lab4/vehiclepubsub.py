...
def customOnMessage(message):
    ...
    print(f"CO2 Max: {json.loads(message.payload)['co2_max']}\n")
...
if args.mode == 'both' or args.mode == 'subscribe':
    myAWSIoTMQTTClient.subscribe(topic+'/results', 0, None)
time.sleep(2)

df = pd.read_csv(f"data2/{thingName}.csv")
...
while True:
    if args.mode == 'both' or args.mode == 'publish':
        ...
        if loopCount < len(data):
            message = data[loopCount]   
